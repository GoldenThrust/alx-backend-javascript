...
describe('API test', () => {
    ...
    it('POST /login returns correct response', (done) => {
        request.post(`${BASE_URL}/login`, { json: { "userName": "Betty" } }, (_err, res, body) => {
            expect(res.statusCode).to.be.equal(200);
    ...
    it('GET /available_payments returns correct response', (done) => {
        request.get(`${BASE_URL}/available_payments`, (_err, res, body) => {
            expect(res.statusCode).to.be.equal(200);
            expect(JSON.parse(body).payment_methods).to.be.deep.equal({ credit_cards: true, paypal: false });
            done();
        });
    });
});